        ...
        it
            .map { (rule, password) ->
                Triple(
                    ...
                    rule.substringBefore(" ").split("-").map { it.toInt() }.let { it.first() to it.last() }, // rule
                    password
                )
            }
            .map { (letter, rules, password) ->
                password.count { it == letter }
                    .let { occurences ->
                        rules.let {
                            if (occurences >= it.first && occurences <= it.second) 1 else 0
                        }
                    }
            }
    }.sum()

fun day2_2(data: String) =  data
    .lines()
    .map { it.split(":").let { it.first() to it.last() } }
    .let {
        it.map { (rule, password) ->
            rule.split(" ")
                .let { it.first().split("-").let { it.first().toInt() to it.last().toInt() } to it.last()[0] }
                .let { (occurrences, expectedLetter) ->
                    password.toCharArray()
                        .filterIndexed { index, _ -> index == occurrences.first || index == occurrences.second }
                        .fold(0) { res, letter ->
                            if (letter == expectedLetter) res + 1 else res
                        }
                }
        }.filter { it == 1 }
            .count()
...
fun main() {
    ...
    day2_1(data2).apply { println(this) }
    day2_2(data2).apply { println(this) }
}


val data2 = """1-7 j: vrfjljjwbsv
1-10 j: jjjjjjjjjjjj
