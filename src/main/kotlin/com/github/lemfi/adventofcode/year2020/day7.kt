...
infix fun String.`contains how much`(bags: List<Bag>): Int {
    return bags.firstOrNull {
        ...
        it?.containedBags?.fold(it
                .containedBags
                .map { it.first }
                .sum()
        ) { res, current -> res + current.first * current.second.`contains how much`(bags) }
                ?: 0
    }
}
...
val day7data = """vibrant aqua bags contain 1 shiny magenta bag, 2 muted teal bags, 1 dim magenta bag, 1 muted chartreuse bag.
